...
if [ "$file" = "Found" ];then
	sudo gzip -9 /var/log/pihole-FTL.log.1 && sudo cp -p /var/log/pihole-FTL.log.1.gz /var/log/pihole/pihole-FTL.log.`date -d '-1 day' +"%Y-%m-%d"`.gz && sudo rm -f /var/log/pihole-FTL.log.1.gz || echo -e "Subject:Error during pihole FTL log rotation\n\nHello,\n\nThere was an error during pihole FTL log rotation." | /usr/sbin/sendmail yourmailexample@mail.com, yourmailexample2@mail.com
